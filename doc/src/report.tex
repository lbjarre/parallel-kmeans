...
\Cref{lloyds} describes this procedure in pseudocode.

\begin{algorithm}[!ht]
  
  \BlankLine
  \KwIn{Data points $\mathcal{X} = \{\bm{x}_1, \bm{x}_2, \ldots, \bm{x}_n\}$
        with $\bm{x}_i \in \mathbb{R}^d$,
        number of clusters $k$}
  \KwOut{Class means $\bm{\mu}_1,\bm{\mu}_2,\ldots,\bm{\mu}_k$}
  \BlankLine

  \While{$\forall \bm{\mu}_k \neq \bm{\mu}_k^{(new)}$}{
    \For{$\forall \bm{x} \in \mathcal{X}$}{
      class $\gets$ $\min_k ||\bm{x} - \bm{\mu}_k||^2$\;
      count[class]++\;
      $\bm{\mu}_{class}^{(new)} \gets \bm{\mu}_{class}^{(new)} + \bm{x}$\;
    }
    \For{$i=1,\ldots,k$}{
      $\bm{\mu}_k^{(new)} \gets \frac{\bm{\mu}_k^{(new)}}{\text{count}[i]}$\;
    }
  }
